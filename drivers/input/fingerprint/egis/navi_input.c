...
#define	KEYEVENT_CLICK              KEY_FPS_TAP
...
#define	KEYEVENT_LONGTOUCH          KEY_FPS_HOLD
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (schedule_work(&(navi_work_queue.workq)) == 0)
        pr_err("Egis navigation driver, fp is NULL\n");
...
void uinput_egis_destroy(struct etspi_data *etspi)
{
    ...
    destroy_workqueue((struct workqueue_struct*)&(navi_work_queue.workq));
...
void sysfs_egis_init(struct etspi_data *etspi)
{
	...
	DEBUG_PRINT("20180709 Egis navigation driver, egis_input device init\n");
