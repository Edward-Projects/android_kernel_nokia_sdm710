...
{                                                                                   \
    ...
    if (ret < 0) {                                                                  \
        pr_debug("wlc: Fail read %s\n", __func__);                                  \
        return 0;                                                                   \
    }                                                                               \
    pr_err("wlc: %s[1][0]: 0x%x 0x%x\n", __func__, func_name[1], func_name[0]);   \
                                                                                    \
    return *p_## func_name;                                                         \
}
...
static int p9221_wlc_remove(struct i2c_client *client)
{
 ...
 static struct i2c_driver p9221_wlc_driver = {
    ...
    //.of_match_table = p9221_wlc_match_table,
		.of_match_table = of_match_ptr(p9221_wlc_match_table),
#ifdef CONFIG_PM
    .pm = &p9221_wlc_pm_ops,
#endif
    },
 ...
 static int __init p9221_wlc_init(void)
{
	...
	ret = i2c_add_driver(&p9221_wlc_driver);
	if (ret)
