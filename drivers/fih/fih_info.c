...
static char variant[8];
char to_ois_baseband[8];
...
static int fih_info_proc_show_variant(struct seq_file *m, void *v)
{
	seq_printf(m, "%s\n", variant);
	return 0;
}

static int fih_info_proc_open_variant(struct inode *inode, struct file *file)
{
	return single_open(file, fih_info_proc_show_variant, NULL);
}

static const struct file_operations fih_info_fops_variant = {
	.open    = fih_info_proc_open_variant,
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = single_release,
};
...
static int fih_info_property(struct platform_device *pdev)
{
	...
	if (!p_chr) {
		...
		strlcpy(to_ois_baseband, p_chr, sizeof(baseband));
	...
	p_chr = of_get_property(pdev->dev.of_node, "fih-info,variant", NULL);
	if (!p_chr) {
		pr_info("%s:%d, variant not specified\n", __func__, __LINE__);
	} else {
		strlcpy(variant, p_chr, sizeof(variant));
		pr_info("%s: variant = %s\n", __func__, variant);
	}
...
static int fih_info_probe(struct platform_device *pdev)
{
	...
	proc_create("variantid",  0, NULL, &fih_info_fops_variant);
...
{
	remove_proc_entry ("variantid",  NULL);
...
static void fih_info_version(void)
{
  ...
  if (strstr(saved_command_line,"androidboot.skuid=600ID")) {
