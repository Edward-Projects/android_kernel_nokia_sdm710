...
static int tas2557_dac_volume_get(struct snd_kcontrol *pKcontrol,
	struct snd_ctl_elem_value *pValue)
{
#ifdef KCONTROL_CODEC
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(pKcontrol);
	...
	if (ret >= 0)
		pValue->value.integer.value[0] = ((nValue&TAS2557_DAC_GAIN_MASK)>>TAS2557_DAC_GAIN_SHIFT);

	dev_info(pTAS2557->dev, "tas2557_dac_volume_get = %d\n", nValue);

	mutex_unlock(&pTAS2557->codec_lock);
	return 0;
}

static int tas2557_dac_volume_put(struct snd_kcontrol *pKcontrol,
	struct snd_ctl_elem_value *pValue)
{
#ifdef KCONTROL_CODEC
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(pKcontrol);
...
static const struct snd_kcontrol_new tas2557_snd_controls[] = {
	...
	SOC_SINGLE_EXT("SAMP DAC", SND_SOC_NOPM, 0, 0x000F, 0,
		tas2557_dac_volume_get, tas2557_dac_volume_put),
};
