...
static struct delayed_work		update_tz_info_work;
...
static struct show_thermal_sensor fih_therm_sensor_list[] = 
            { {"CPU0", "cpu0-silver-usr", 100},
              {"CPU1", "cpu1-silver-usr", 100},
              {"CPU2", "cpu2-silver-usr", 100},
              {"CPU3", "cpu3-silver-usr", 100},
              {"CPU4", "cpu4-silver-usr", 100},
              {"CPU5", "cpu5-silver-usr", 100},
              {"CPU6", "cpu0-gold-usr", 100},
              {"CPU7", "cpu1-gold-usr", 100},
              {"GPU0", "gpu0-usr", 100},
              {"GPU1", "gpu1-usr", 100},
              {"DSP", "mdm-dsp-usr", 100},
              {"WLAN", "wlan-usr", 100},
              {"CAMERA", "camera-usr", 100},
              {"MMSS", "mmss-usr", 100},
              {"DDR", "ddr-usr", 100},
              {"MODEM", "mdm-core-usr", 100},
              {"QUIET", "quiet-therm-adc", 100},
              {"BAT", "battery", 100},
              {"PM660", "pm660_tz", 100},
              {"PM660L", "pm660l_tz", 100},
              {"XO", "xo-therm-adc", 100},
              {"PA", "pa-therm0-adc", 100},
              {"MSM", "msm-therm-adc", 100},
            };
...
static int debug_cpu_usage_enable_set(const char *val, const struct kernel_param *kp)
{
...
static int debug_cpu_usage_interval_set(const char *val, const struct kernel_param *kp)
{
	int ret;
...
extern void quick_get_limits_dcvs_freq(unsigned int *curr);
...
static void get_cpu_usage_and_freq(char * cpu_info_msg, int msg_len)
{
	...
	unsigned int limits_dcvs[2] = {0, 0};
	long cpu_usage = 0;
	long iowait_usage = 0;
	...
	quick_get_limits_dcvs_freq(limits_dcvs);
	tmp_len = snprintf((cpu_info_msg + str_len), len, "[LIMITS0=%u][LIMITS1=%u]",limits_dcvs[0]/1000, limits_dcvs[1]/1000);
	str_len += tmp_len;
	len -= tmp_len;
	...
	tmp_len = snprintf((cpu_info_msg + str_len), len, "[GPU min=%u max=%u curr=%u therm=%u pct=%u%%]\n%s:",
				gpu_freq_min, gpu_freq_max, gpu_curr_freq, gpu_therm_freq, gpu_usage, cpu_usage_tag);
	str_len += tmp_len;
	len -= tmp_len;
